...
/*mp.events.add('fishingBaitTaken', () => {
    buttonstate = randomInteger(0, 4);
    color = randomInteger(150, 255);
    fishing = true;
});

mp.events.add('stopfish', () => {
    fishing = false;
    fishingSuccess = 0;
    color = -1;
});
...
mp.keys.bind(Keys.VK_W, false, function () { // Fish key
    if (buttonstate == 0 && press == false)
        press = true;
});

mp.keys.bind(Keys.VK_Z, false, function () { // Fish key
    if (buttonstate == 1 && press == false)
        press = true;
});

mp.keys.bind(Keys.VK_R, false, function () { // Fish key
    if (buttonstate == 2 && press == false)
        press = true;
});

mp.keys.bind(Keys.VK_H, false, function () { // Fish key
    if (buttonstate == 3 && press == false)
        press = true;
});

mp.keys.bind(Keys.VK_B, false, function () { // Fish key
    if (buttonstate == 4 && press == false)
        press = true;
});


var buttons = ["W","Z","R","H","B"];

function drawFishingMinigame() {

        mp.game.graphics.drawText(buttons[buttonstate], [0.47, 0.47], { 
            font: 2, 
            color: [color, 255, 255, 220], 
            scale: [1.2, 1.2], 
            outline: true
          });
...
//mp.keys.bind(Keys.VK_Z, false, function () {
  // mp.events.callRemote('sc');
//});

// CRUISE CONTROL //

/*mp.keys.bind(Keys.VK_6, false, function () { // 6 key - cruise mode on/off
