    ...
    if (showGamertags) {
        ...
        nametags.forEach(nametag => {
            ...
            if(player.getVariable('INVISIBLE') != true && player.getVariable('HideNick') != true)
            {
				...
				text = player.getVariable('IS_MASK') ? ( global.localplayer.getVariable('IS_ADMIN') || localFraction != null && playerFraction != null && localFraction === playerFraction || global.localplayer.getVariable('familycid') != null && player.getVariable('familycid') != null && global.localplayer.getVariable('familycid') == player.getVariable('familycid') ? player.name + ' [' + tag + ']' + '\n в маске' : 'Гражданин [' + tag + '] \n в маске') : (global.localplayer.getVariable('IS_ADMIN') || localFraction != null && playerFraction != null && localFraction === playerFraction || mp.storage.data.friends[player.name] ? player.name + ' [' + tag + ']' : 'Гражданин [' + tag + ']' );
...
function drawPlayerVoiceIcon(player, x, y)
{
    ...
    if (player.isVoiceActive) drawSprite("mpleaderboard", sprite, [0.7, 0.7], 0, [0, 94, 217, 255], x, y - 0.02 * 0.7);
}
